...
class SmokeAnnotationGUI:
    def __init__(self, root):
        ...
        # Grid overlay variables for region-based annotation
        self.grid_info = {}
        self.selected_regions = set()  # Track which regions are marked as having smoke
    ...
    def setupVideoDisplay(self, parent):
        ...
        # Bind canvas click for grid region selection
        self.videoCanvas.bind('<Button-1>', self.on_canvas_click)
    ...
    def setupControlPanel(self, parent):
        ...
        instructionsLabel = tk.Label(self.frameAnnotationFrame, 
                                     text="Click regions on the image grid,\nor use Smoke/No Smoke buttons", 
    ...
    def previousFrame(self):
        ...
        # Save current frame's region data before moving
        self.saveCurrentFrameRegions()
        
        self.currentFrameIndex -= 1
        
        # Load new frame's region data
        self.loadCurrentFrameRegions()
    ...
    def nextFrame(self):
        ...
        if not self.isCurrentFrameAnnotated():
            messagebox.showwarning("Annotation Required", 
                                 ...
                                 "Click regions or use 'Smoke'/'No Smoke' buttons to continue.")
            return
    ...
    def setFrameAnnotationMode(self):
        ...
        if self.frameFiles:
            ...
            # Load current frame's region data
            self.loadCurrentFrameRegions()
            self.displayCurrentFrame()
            self.updateInfoDisplay()
            self.updateNavigationButtons()
    ...
    def markSmoke(self):
        """Mark current frame as having smoke (select all regions) and advance to next frame"""
        if self.workflowState != "frameAnnotation" or not self.frameFiles:
            return
            
        # Select all 9 regions as having smoke
        self.selected_regions = {1, 2, 3, 4, 5, 6, 7, 8, 9}
        
        # Save current frame's region data
        self.saveCurrentFrameRegions()
        
        # Refresh display to show all regions selected
        self.displayCurrentFrame()
    ...
    def markNoSmoke(self):
        """Mark current frame as no smoke (deselect all regions) and advance to next frame"""
        if self.workflowState != "frameAnnotation" or not self.frameFiles:
            return
            
        # Deselect all regions (no smoke)
        self.selected_regions = set()
        
        # Save current frame's region data
        self.saveCurrentFrameRegions()
        
        # Refresh display to show no regions selected
        self.displayCurrentFrame()
    ...
    def displayCurrentFrame(self):
        """Display current frame with 3x3 grid overlay for region annotation"""
        ...
        try:
            ...
            # Add 3x3 grid overlay if in frame annotation mode
            if self.workflowState == "frameAnnotation":
                self.draw_grid_overlay(x, y, newWidth, newHeight)
            ...
            if frameName in self.frameAnnotations:
                regions = self.frameAnnotations[frameName]
                if isinstance(regions, list):
                    if len(regions) == 0:
                        annotationText = " (NO SMOKE)"
                    elif len(regions) == 9:
                        annotationText = " (ALL SMOKE)"
                    else:
                        annotationText = f" (REGIONS: {sorted(regions)})"
                else:
                    # Legacy binary format
                    annotationText = " (SMOKE)" if regions else " (NO SMOKE)"
    ...
    def draw_grid_overlay(self, img_x, img_y, img_width, img_height):
        """Draw a 3x3 grid overlay on the image with green outlines for selected regions"""
        ...
        # Draw black grid lines first
        for i in range(1, 3):
            # Vertical lines
            x = img_x + i * cell_width
            self.videoCanvas.create_line(x, img_y, x, img_y + img_height, 
                                        fill='black', width=2, tags='grid')
        
        for i in range(1, 3):
            # Horizontal lines
            y = img_y + i * cell_height
            self.videoCanvas.create_line(img_x, y, img_x + img_width, y, 
                                        fill='black', width=2, tags='grid')
        
        # Draw green outlines only for selected regions
        for row in range(3):
            for col in range(3):
                region_num = row * 3 + col + 1
                x0 = img_x + col * cell_width
                y0 = img_y + row * cell_height
                x1 = x0 + cell_width
                y1 = y0 + cell_height
                
                # Only draw green outline for selected regions
                if region_num in self.selected_regions:
                    # Create thick green outline
                    self.videoCanvas.create_rectangle(x0, y0, x1, y1, 
                                                     fill='', outline='#4caf50', 
                                                     width=4, tags='grid')

    def on_canvas_click(self, event):
        """Handle clicks on the canvas to select/deselect regions"""
        # Only handle clicks in frame annotation mode
        if self.workflowState != "frameAnnotation" or not hasattr(self, 'grid_info'):
            return
        ...
        grid = self.grid_info
        if (click_x < grid['img_x'] or click_x > grid['img_x'] + grid['img_width'] or
            click_y < grid['img_y'] or click_y > grid['img_y'] + grid['img_height']):
            return
        
        # Calculate which grid cell was clicked
        relative_x = click_x - grid['img_x']
        relative_y = click_y - grid['img_y']
        
        col = relative_x // grid['cell_width']
        row = relative_y // grid['cell_height']
        
        # Ensure we're within bounds
        if 0 <= row < 3 and 0 <= col < 3:
            region_num = row * 3 + col + 1
            self.toggle_region_smoke(region_num)

    def toggle_region_smoke(self, region_num):
        """Toggle smoke annotation for a specific region"""
        if region_num in self.selected_regions:
            self.selected_regions.remove(region_num)
        else:
            self.selected_regions.add(region_num)
        
        # Refresh the display to update the grid overlay
        self.displayCurrentFrame()
        
        # Auto-save after each region selection
        self.autoSave()

    def saveCurrentFrameRegions(self):
        """Save current frame's selected regions to frameAnnotations"""
        if not self.frameFiles or self.currentFrameIndex >= len(self.frameFiles):
            return
        ...
        # Store the selected regions for this frame
        self.frameAnnotations[frameName] = list(self.selected_regions)

    def loadCurrentFrameRegions(self):
        """Load current frame's selected regions from frameAnnotations"""
        if not self.frameFiles or self.currentFrameIndex >= len(self.frameFiles):
            self.selected_regions = set()
            return
        ...
        # Load the selected regions for this frame
        if frameName in self.frameAnnotations:
            self.selected_regions = set(self.frameAnnotations[frameName])
        else:
            self.selected_regions = set()
    ...
    def updateAnnotationButtonColors(self):
        ...
        # Highlight the button based on current regions
        if frameName in self.frameAnnotations:
            regions = self.frameAnnotations[frameName]
            if isinstance(regions, list):
                if len(regions) == 9:  # All regions selected = smoke
                    if hasattr(self, 'smokeBtn'):
                        self.smokeBtn.config(bg='#FF8C00')  # Orange for selected smoke
                elif len(regions) == 0:  # No regions selected = no smoke
                    if hasattr(self, 'noSmokeBtn'):
                        self.noSmokeBtn.config(bg='#FF8C00')  # Orange for selected no smoke
                # Partial regions selected = neither button highlighted
            else:
                # Legacy binary format compatibility
                if regions:  # Smoke
                    if hasattr(self, 'smokeBtn'):
                        self.smokeBtn.config(bg='#FF8C00')  # Orange for selected smoke
                else:  # No smoke
